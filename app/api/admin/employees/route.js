...
const SALON_ROLES = ['stylist', 'barber', 'beautician', 'receptionist', 'admin'];

function authRoleForSalonRole(role) {
...
function validateStaff(data, editing = false) {
  ...
  if (!SALON_ROLES.includes(cleanText(data.salon_role || data.role))) return 'Valid salon role is required';
  if (!editing && !/^\d{4}$/.test(String(data.pin || ''))) return 'PIN must be 4 digits';
  ...
  try {
    ...
    const employees = db.prepare(`
      SELECT u.id, u.username, u.full_name, u.role, u.email, u.phone, u.is_active, u.created_at,
             COALESCE(sp.salon_role, CASE WHEN u.role = 'admin' THEN 'admin' ELSE 'stylist' END) as salon_role,
...
export async function POST(request) {
  try {
    ...
    `).run(
      ...
      authRoleForSalonRole(cleanText(data.salon_role || data.role)),
      hashedPassword,
    ...
    db.prepare(`
      INSERT INTO staff_profiles (user_id, salon_role, assigned_services, commission_percentage, base_salary)
      VALUES (?, ?, ?, ?, ?)
    `).run(
      result.lastInsertRowid,
      cleanText(data.salon_role || data.role),
...
export async function PUT(request) {
  try {
    ...
    if (data.pin) {
      if (!/^\d{4}$/.test(String(data.pin))) return NextResponse.json({ error: 'PIN must be 4 digits' }, { status: 400 });
      sql += ', password_hash = ?';
      params.push(bcrypt.hashSync(String(data.pin), 10));
    }
    sql += ' WHERE id = ?';
    params.push(data.id);
